...
- **Web 管理台**: 前端位于 `frontend/`，`/` 优先使用 Vue 3 + TypeScript + Vite + 自建 UI 组件 + Pinia + Vue Query 新管理台；若 `frontend/dist/index.html` 不存在，`/` 回退到 `frontend/admin.html` 旧单文件管理台。`/admin` 始终使用旧管理台。对于功能变化，需要修改时须一同修改两个管理台；优化类则不用（旧管理台仅保证最小支持）。开发态使用 `pnpm run dev`，Vite 将 `/auth`、`/api`、`/codebuddy`、`/openai`、`/health`、`/docs`、`/redoc` 和 `/openapi.json` 代理到 `127.0.0.1:8001`。
