...
def _normalized_relative_path(value: str, *, context: str) -> str:
    ...
    if path.is_absolute() or any(
        part in ("", ".", "..") or PureWindowsPath(part).drive
    ...
    try:
        ...
        try:
            ...
            raise UpdateError(
                ...
                f"恢复错误：{_describe_failure(restore_error)}；"
                f"安全备份已保留在 {pending}"
            ) from restore_error
        _remove_path(pending)
        if isinstance(error, KeyboardInterrupt):
            raise
        raise UpdateError(
            f"回滚失败，已恢复回滚前状态：{_describe_failure(error)}"
        ) from error
...
def _ensure_execution_context(project_root: Path) -> None:
    ...
    execution_values = [sys.executable]
    if sys.prefix != sys.base_prefix:
        execution_values.append(sys.prefix)
    execution_paths = set()
    for execution_value in execution_values:
        execution_path = Path(os.path.abspath(execution_value))
        execution_paths.update((execution_path, execution_path.resolve()))
    for execution_path in execution_paths:
        for project_path in project_paths:
            try:
                execution_path.relative_to(project_path)
            except ValueError:
                continue
            raise UpdateError(
    ...
    with tempfile.TemporaryDirectory(prefix="codebuddy2api-update-") as temp_dir:
        ...
        try:
            ...
            if isinstance(error, UpdateError):
                raise
            raise UpdateError(
                f"更新失败，已自动恢复：{_describe_failure(error)}"
            ) from error
...
def _confirm_operation(action: str) -> None:
    while True:
        answer = _read_interactive_answer(
            ...
            "（输入 y 并按 Enter 键继续；直接按 Enter 键或输入 n 并按 Enter 取消）："
        )
        if answer in ("y", "yes"):
            return
        if answer in ("", "n", "no", "q"):
