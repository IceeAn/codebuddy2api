...
export function quotaTone(quota: CredentialQuota): CredentialQuotaTone {
  ...
  if (percent === null || quota.status === 'unknown' || quota.status === 'error') return 'muted';
...
export function formatQuotaTooltipSections(quota: CredentialQuota): string[] {
  if (quota.status === 'unknown') return ['额度尚未探测'];
...
const label = computed(() => {
  if (props.quota.status === 'unknown') return '额度尚未探测';
  if (
  ...
  return `${prefix} ${props.quota.remaining} / ${props.quota.total}，${percent.value}%${props.quota.status === 'stale' ? '，数据已陈旧' : ''}`;
});
