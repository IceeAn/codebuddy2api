...
describe('ApiConsoleView', () => {
  ...
  it('切换 Anthropic 后隔离模型查询、转换请求并清空进行中输出', async () => {
    ...
    const anthropicQueryKey = queryOptions.value.queryKey.value;
    await expect(
      queryOptions.value.queryFn({
        queryKey: anthropicQueryKey,
        signal: modelController.signal,
      }),
    ).resolves.toEqual({ data: [{ id: 'anthropic/codebuddy/glm' }] });
    expect(anthropicModelsMock).toHaveBeenCalledWith(modelController.signal);
    state.protocol = 'openai';
    await wrapper.vm.$nextTick();
    const openaiQueryKey = queryOptions.value.queryKey.value;
    state.protocol = 'anthropic';
    await wrapper.vm.$nextTick();
    await queryOptions.value.queryFn({
      queryKey: openaiQueryKey,
      signal: modelController.signal,
    });
  ...
  it('请求与响应卡片固定上下排列，模型和操作控件按要求对齐', () => {
    ...
    const protocolGroup = wrapper.get('[aria-label="协议"]');
    expect(protocolGroup.classes()).toContain('self-start');
