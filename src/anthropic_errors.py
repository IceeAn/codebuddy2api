...
def is_anthropic_path(path: str) -> bool:
    """识别外部与管理台 playground 的完整 Anthropic 命名空间。"""
    return (
        path == "/anthropic"
        or path.startswith("/anthropic/")
        or path == "/api/admin/playground/anthropic"
        or path.startswith("/api/admin/playground/anthropic/")
    )
