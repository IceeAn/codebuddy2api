...
def _reject_nonstandard_json_constant(constant: str) -> NoReturn:
    """拒绝 Python JSON 解码器默认接受的非标准数值常量。"""
...
class _AnthropicEventConsumer:
    ...
    def _validate_legacy_function_call(event: CodeBuddyResponseEvent) -> None:
        if "function_call" not in event.delta:
            return
        value = event.delta["function_call"]
        if value is None or value == {}:
            return
        if not isinstance(value, dict):
            raise UpstreamProtocolViolation(
                "CodeBuddy legacy function_call delta is not supported"
            )
        empty_values = (None, "", [], {})
        if all(item in empty_values for item in value.values()):
            return
        raise UpstreamProtocolViolation(
            "CodeBuddy legacy function_call delta is not supported"
        )
...
class AnthropicNonStreamAggregator(_AnthropicEventConsumer):
    ...
    def __init__(self, context: AnthropicResponseContext):
        ...
        self.pending_tool_indexes: set[int] = set()
...
class AnthropicStreamEncoder(_AnthropicEventConsumer):
    ...
    def _flush_tool_group(self) -> List[str]:
        states = [
            self.tools[index]
            for index in sorted(self.pending_tool_indexes)
        ]
...
def map_anthropic_error(error: Any) -> tuple[int, str, str]:
    ...
    if not isinstance(status_code, int) or not 400 <= status_code <= 599:
        raise ValueError("Anthropic error status_code must be an HTTP error status")
    if upstream_status is not None:
        if not isinstance(upstream_status, int) or not 100 <= upstream_status <= 599:
            raise ValueError(
                "Anthropic error upstream_status_code must be a valid HTTP status"
            )
        effective_status = upstream_status if upstream_status >= 400 else status_code
    else:
        effective_status = status_code

    if effective_status == 504 or category == "upstream_timeout":
        return 504, "timeout_error", message
    if effective_status == 429:
        return 429, "rate_limit_error", message
    if effective_status == 529:
    ...
    if category in {"no_credential", "internal_error"}:
        return 500, "api_error", message
    if effective_status < 500:
        return effective_status, "invalid_request_error", message
    return effective_status, "api_error", message
