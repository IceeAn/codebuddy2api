...
class ReleaseRuntimeLockTests(unittest.TestCase):
    ...
    def test_service_rejects_markerless_installation_after_locking(self):
        (self.root / release_runtime_lock.LOCK_FILENAME).write_bytes(b"\0")
        service_lock = None
        try:
            with self.assertRaisesRegex(
                release_runtime_lock.RuntimeLockError,
                "Release 清单",
            ):
                service_lock = release_runtime_lock.acquire_runtime_lock(
                    self.root,
                    required=False,
                    purpose="服务",
                )
        ...
        recovery_lock = release_runtime_lock.acquire_runtime_lock(
            self.root,
            required=True,
            purpose="回滚",
        )
    ...
    def test_required_lock_recovers_damaged_markers_with_existing_lock(self):
        (self.root / "VERSION").write_text("v1.2.3\n", encoding="utf-8")
        with self.assertRaisesRegex(release_runtime_lock.RuntimeLockError, "不完整"):
            release_runtime_lock.acquire_runtime_lock(
                self.root,
